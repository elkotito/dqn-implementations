...
class DoubleDQNTrainer:
    ...
    This trainer owns the replay-DQN training loop. Reusable training utilities
    ...
    def _gradient_update(self, transitions: Transitions) -> GradientUpdateMetrics:
        ...
        with torch.no_grad():
            ...
            double_q_values = self.target_network(next_states).gather(index=next_actions[:, None], dim=1).squeeze(1)
            target_q_values = rewards + self.config.gamma * (~dones).float() * double_q_values
